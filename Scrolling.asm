...
; UPDATE SCROLLING
;-------------------------------------------------------------------------------
; Entry point for all scrolling. Here we work out what direction (if any) we are 
; scrolling in
; and branch off to the directional scroll routines - there we divide the work 
; up along the
...
;===============================================================================
; The screen has stopped moving here and we exit
;===============================================================================

        lda SCROLL_MOVING               ; If we are stoppped (SCROLL_STOP = 0) 
                                        ;then we can return
        bne @fix_frame
        rts

;===============================================================================
; The screen is still moving here
; So we initialize all these checks below
;-------------------------------------------------------------------------------
;===============================================================================
; Check if SCROLL_MOVING <> 1 (SCROLL_RIGHT)
;===============================================================================
@fix_frame
@fix_right
...
;===============================================================================
; SCROLL_COUNT_X = increases from 0-7 to keep the tile scrolling smooth (in pixels)

; Screen is moving to the right (SCROLL_MOVING), so we check if
; SCROLL_COUNT_X = 4 and goto @fix_done which will set 
; SCROLL_MOVING = 0 (SCROLL_STOP) and then it checks for a screen scroll
...
;                               ; Do our extra pixel scroll adjustment
        ...
        jsr MoveSpriteLeft      ; Eventually we'll scroll all the 
                                ;active sprites
        ldx #1
        jsr MoveSpriteLeft
...
;===============================================================================
; Check if SCROLL_MOVING <> 1 (SCROLL_LEFT)
;===============================================================================
@fix_left
        cmp #SCROLL_LEFT
        bne @fix_up
...
;===============================================================================
; Fix the scroll to the left
;===============================================================================
;===============================================================================
; Screen is moving to the left (SCROLL_MOVING). 
; First check if SCROLL_FIX_SKIP <>0 and goto @fix_done.
; Then we check if
; SCROLL_COUNT_X = 4 and goto @fix_done which will set 
; SCROLL_MOVING = 0 (SCROLL_STOP) and then it checks for a screen scroll
; to the RIGHT, LEFT, DOWN, and UP.
...
;===============================================================================
; SCROLL_COUNT_X = increases from 0-7 to keep the tile scrolling smooth (in pixels)
; SCROLL_MOVING = checks if the screen is still scrolling.
...
;===============================================================================
; Fix the the up scroll
;===============================================================================
;===============================================================================
; Check if SCROLL_MOVING <> 1 (SCROLL_UP)

; SCROLL_COUNT_Y = increases from 0-7 to keep the tile scrolling smooth (in pixels)
;===============================================================================
@fix_up
        cmp #SCROLL_UP
        bne @fix_down
...
;===============================================================================        
;===============================================================================
; Check if SCROLL_MOVING <> 1 (SCROLL_DOWN)
...
;===============================================================================
; This section is called anytime SCROLL_DIRECTION <> 0
; Then it will check SCROLL_MOVING for each direction: right, left, down & up
;===============================================================================
@start
...
@frame5
        cmp #5
        bne @frame6
;        jsr CopyHorizontalBuffer        ; copy new characters from the map to 
                                         ;the buffers
        jsr CopyLeftBuffer
        rts
@frame6 
...
@frame0
        ...
        sec
        lda MAP_X_DELTA                         ; decrement map X delta by 3 
                                                ;(character within tile)
        sbc #1
...
@new_tile
        ...
        sbc #1                                  ; (address of the top - right
        sta MAP_ADDRESS                         ; corner of the map)
        lda MAP_ADDRESS + 1
        sbc #0
        sta MAP_ADDRESS + 1
...
ScrollRight
        ...
        sta SCROLL_COUNT_X      ; store the new count - Raster IRQ does the 
                                ; ScrollRight

                                ; A holds the count, from here we can test against it
                                ; to split the workload depending on what 
                                ; 'frame' we are at
...
; SHIFT CHARS LEFT 
;-------------------------------------------------------------------------------
; Shift the characters for a scroll in the left direction (characters are 
; moving right) routines are unrolled for speed
;-------------------------------------------------------------------------------
...
; DRAW UP COLOR
;-------------------------------------------------------------------------------
; Write the contents of the VERTICAL_COLOR_BUFFER to the top 'off screen' line 
; in color ram
;-------------------------------------------------------------------------------
#region "DrawUpColor"
DrawUpColor
...
; DRAW DOWN COLOR
;-------------------------------------------------------------------------------
; Write the contents of the VERTICAL_COLOR_BUFFER to the bottom 'off screen' 
; line in color ram
;-------------------------------------------------------------------------------
...
;-------------------------------------------------------------------------------
; Copy the data needed to the VERTICAL_BUFFER and VERTICAL_COLOR_BUFFER for 
; edge drawing of new characters on the 'jump frame'
; 
; Note : this will have to be rewritten to combine vert and horiz scrolling and 
; delta values
;-------------------------------------------------------------------------------
...
CopyVerticalBuffer
        ...
        lda MAP_Y_POS
                                ...
                                ; The direction will dictate how we calculate 
                                ; what tile 
        ...
        ;-----------------------------------------------------------------------
                                ; DIRECTION UP
                                ; Scrolling up.  The tile we need is the same 
                                ; if MAP_Y_DELTA
...
@scrollingUp
        ...
        ;-----------------------------------------------------------------------
                                ...
                                ; and the character line within the tile is 
                                ; MAP_Y_DELTA + 2
                                ; This means we may be reading from tile 
                                ; MAP_Y + 5 if the
                                ; MAP_Y_DELTA >= 2
                                ; So if we add 2 then and %0011 (3) to it, to 
                                ; mask it 
                                ; back to a 0-3 value.. if our value > MAP_Y_DELTA 
                                ; we are in the 
...
@tileloop
        ...
        sta ZEROPAGE_POINTER_2          ; store the map line address in
        lda MAP_LINE_LOOKUP_HI,x        ; ZEROPAGE_POINTER_2
        sta ZEROPAGE_POINTER_2 + 1

                                        ; Next use the adjusted MAP_X_POS to
        ldy PARAM1                      ; get the tile
        lda (ZEROPAGE_POINTER_2),y      ; Fetch the tile number in A

        tax
        lda TILE_NUMBER_LOOKUP_LO,x     ; use the tile number to lookup the
        sta ZEROPAGE_POINTER_1          ; address of the tile data
        lda TILE_NUMBER_LOOKUP_HI,x
        sta ZEROPAGE_POINTER_1 + 1

;-----------------------------------------------------------------------
; We now have the address to the tile in ZEROPAGE_POINTER_1, so now we 
; have to
...
;
; Actually it turned out so well that I decided to rewrite the original 
; CopyHorizontalBuffer
; routine, as this version will be much easier to integrate handling MAP_Y_DELTA 
; when I include vertical scrolling
;-------------------------------------------------------------------------------
...
@copyloop
        ...
        sta HORIZONTAL_COLOR_BUFFER,x   ; so we can save the attribute in the 
                                        ; color buffer
...
SCROLL_FIX_SKIP                        ; Check to see if we can 'skip'
        byte $0                        ; scroll fixing on stop
